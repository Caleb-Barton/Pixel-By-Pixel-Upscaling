...
OUTPUT_PATH = "data/kis_model_5x5/"
...
def extract_windows(small_img, large_img):
    """
    Extract all 5x5 windows from the small image and their corresponding
    3x3 output regions from the large image.
    
    Returns two lists:
    - input_windows: list of flattened 25-element arrays (5x5 input windows)
    - output_windows: list of flattened 9-element arrays (3x3 output regions)
    
    Skips homogeneous windows (where all 25 input pixels have the same value).
    """
    height, width = small_img.shape
    input_windows = []
    output_windows = []

    homogeneous_add_count = 0
    
    # Iterate through each pixel in the small image (skip 2-pixel border for 5x5 windows)
    for y in range(2, height - 2):
        for x in range(2, width - 2):
            # Extract 5x5 window from small image centered at (y, x)
            input_window = small_img[y-2:y+3, x-2:x+3]
    ...
    print(f"  Added {homogeneous_add_count} homogeneous windows")
...
def generate_dataset(image_list, small_dir, large_dir):
    ...
    Returns two numpy arrays: inputs (N, 25) and outputs (N, 9).
    """
    all_inputs = []
    all_outputs = []
